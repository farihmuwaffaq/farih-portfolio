...
const work = defineCollection({
  ...
  schema: z.object({
    ...
    reportEvidence: z.object({
      eyebrow: z.string().trim().min(1), heading: z.string().trim().min(1), description: z.string().trim().min(1), attribution: z.string().trim().min(1),
      items: z.array(z.object({
        brand: z.string().trim().min(1), period: z.string().trim().min(1), reportType: z.string().trim().min(1), summary: z.string().trim().min(1),
        metrics: z.array(z.object({ value: z.string().trim().min(1), label: z.string().trim().min(1), context: z.string().trim().min(1) })).min(2).max(3),
        contribution: z.string().trim().min(1), insight: z.string().trim().min(1), caveat: z.string().trim().min(1),
      })).length(3),
    }).optional(),
    dashboardEvidence: z.object({
